  ...
  function click(cell: Cell) {
    ...
    if (selectedCell && selectedCell !== cell && cell.color !== "black" && selectedCell?.figure?.canMove(cell)) {
      ...
      if (((selectedCell.y - cell.y) > 1 && (selectedCell.x - cell.x) < -1 && cell?.figure?.color === 'white') || ((cell.x - selectedCell.x) > 1 && (cell.y - selectedCell.y) > 1 && cell?.figure?.color === 'black')) {
        ...
        if (((cell.x - selectedCell.x) > 2 && (cell.y - selectedCell.y) > 2 && cell?.figure?.color === 'black')) {
          for (let i = cell.x - 1; i > selectedCell.x; i--) {
            if((cell.y - 1) >= 0 ? cell.board.getCell(cell.x - 1, cell.y - 1)?.isFigure() : false){
              banOnHitting = true;
              okOne_two += 1;
            } 
            board.getCell(selectedCell.x, selectedCell.y).deleteFigure(cell, i, cell?.figure?.color === 'black' ?  cell.y + directionEmpty-- : cell.y + directionEmpty++);
          }
          if(okOne_two < 1){
            banOnHitting = false;
            okOne_two = 0;
      ...
      if (((selectedCell.y - cell.y) > 1 && (selectedCell.x - cell.x) > 1 && cell?.figure?.color === 'black') || ((selectedCell.y - cell.y) < -1 && (selectedCell.x - cell.x) > 1 && cell?.figure?.color === 'white')) {
        ...
        if(((selectedCell.y - cell.y) < -2 && (selectedCell.x - cell.x) > 2 && cell?.figure?.color === 'white')) {
          ...
          for (let i = cell.x + 1; i < selectedCell.x; i++) {
            ...
            board.getCell(selectedCell.x, selectedCell.y).deleteFigure(cell, i, cell?.figure?.color === 'white' ?  cell.y + directionEmptyTwo-- : cell.y + directionEmptyTwo++);
          }    
          if(ok_two < 1){
            banOnHitting = false;
            ok_two = 0;
      ...
      if (((selectedCell.x - cell.x) < -1 && (selectedCell.y - cell.y) > 1 && cell?.figure?.color === 'black') || ((cell.x - selectedCell.x) > 1 && (cell.y - selectedCell.y) > 1 && cell?.figure?.color === 'white')) {
        ...
        if (((cell.x - selectedCell.x) > 2 && (cell.y - selectedCell.y) > 2 && cell?.figure?.color === 'white')) {
          for (let i = cell.x - 1; i > selectedCell.x; i--) {
            if((cell.y - 1) >= 0 ? cell.board.getCell(cell.x - 1, cell.y - 1)?.isFigure() : false){
              banOnHitting = true;
              okThree += 1;
      ...
      // }
      // if (((selectedCell.y - cell.y) > 1 && (selectedCell.x - cell.x) > 1 && cell?.figure?.color === 'black') || ((selectedCell.y - cell.y) < -1 && (selectedCell.x - cell.x) > 1 && cell?.figure?.color === 'white')) {
      //   console.log('ok',((selectedCell.y - cell.y) > 2 && (selectedCell.x - cell.x) > 2 && cell?.figure?.color === 'black'),((selectedCell.y - cell.y) < -2 && (selectedCell.x - cell.x) > 2 && cell?.figure?.color === 'white'))
      //   if (((selectedCell.y - cell.y) > 2 && (selectedCell.x - cell.x) > 2 && cell?.figure?.color === 'black')) {
      //     for (let i = cell.x + 1; i < selectedCell.x; i++) {
      //       if((cell.y - 1) >= 0 ? cell.board.getCell(cell.x + 1, cell.y + 1)?.isFigure()?.color === 'white' : false){
      //         console.log(cell.board.getCell(cell.x + 1, cell.y + 1)?.isFigure(), '11111111111111')
      //         banOnHitting = true;
      //         ok += 1;
      //         console.log(ok, '11111111111111')
      //         console.log(banOnHitting, '11111111111111')
      //       } 
      //       board.getCell(selectedCell.x, selectedCell.y).deleteFigure(cell, i, cell?.figure?.color === 'black' ? cell.y + directionEmptyTwo++ : cell.y + directionEmptyTwo--);
      //       // ok = 0;
      //     }    
      ...
      //     for (let i = cell.x + 1; i < selectedCell.x; i++) {
      //       if((cell.y - 1) >= 0 ? cell.board.getCell(cell.x + 1, cell.y - 1)?.isFigure()?.color === 'black' : false){
      //         console.log(cell.board.getCell(cell.x + 1, cell.y - 1)?.isFigure(), '222222222')
      //         banOnHitting = true;
      //         ok_two += 1;
      //         console.log(ok_two, '2222222')
      //       } 
      //       board.getCell(selectedCell.x, selectedCell.y).deleteFigure(cell, i, cell?.figure?.color === 'white' ?  cell.y + directionEmptyTwo-- : cell.y + directionEmptyTwo++);
      //       // ok_two = 0;
      //     }    
      //     console.log(ok_two, '22222222222')
      //     // console.log(banOnHitting)
      //     if(ok_two < 1){
      //       banOnHitting = false;
      //       ok_two = 0;
      //     }  
    ...
    if (selectedCell === cell && selectedCell?.figure?.name !== 'королева белая шашка' && selectedCell?.figure?.name !== 'королева черная шашка' && !(selectedCell?.figure?.canMove(cell))) {

      if (
        ((!((cell.y + 1) <= 7) ? false : (cell.board.getCell(cell.x + 1, cell.y + 1)?.isFigure()?.color === 'white') === false ? true : cell.board.getCell(cell.x + 1, cell.y + 1)?.isFigure()?.color === 'white') && ((cell.y + 1) <= 7 ? !cell.board.getCell(cell.x + 1, cell.y + 1)?.isEmpty() : null) && !((cell.y + 2) <= 7 ? cell.board.getCell(cell.x + 2, cell.y + 2)?.isEmpty() : null))
        && ((!((cell.y + 1) <= 7) ? false : (cell.board.getCell(cell.x - 1, cell.y + 1)?.isFigure()?.color === 'white') === false ? true : cell.board.getCell(cell.x - 1, cell.y + 1)?.isFigure()?.color === 'white') && ((cell.y + 1) <= 7 ? !cell.board.getCell(cell.x - 1, cell.y + 1)?.isEmpty() : null) && !((cell.y + 2) <= 7 ? cell.board.getCell(cell.x - 2, cell.y + 2)?.isEmpty() : null))
      ...
      if (
        ((!((cell.y - 1) >= 0) ? false : (cell.board.getCell(cell.x + 1, cell.y - 1)?.isFigure()?.color === 'black') === false ? true : cell.board.getCell(cell.x + 1, cell.y - 1)?.isFigure()?.color === 'black') && ((cell.y - 1) >= 0 ? !cell.board.getCell(cell.x + 1, cell.y - 1)?.isEmpty() : null) && !((cell.y - 2) >= 0 ? cell.board.getCell(cell.x + 2, cell.y - 2)?.isEmpty() : null))
        && ((!((cell.y - 1) >= 0) ? false : (cell.board.getCell(cell.x - 1, cell.y - 1)?.isFigure()?.color === 'black') === false ? true : cell.board.getCell(cell.x - 1, cell.y - 1)?.isFigure()?.color === 'black') && ((cell.y - 1) >= 0 ? !cell.board.getCell(cell.x - 1, cell.y - 1)?.isEmpty() : null) && !((cell.y - 2) >= 0 ? cell.board.getCell(cell.x - 2, cell.y - 2)?.isEmpty() : null))
            ...
            <div className="checkers__content">
              ...
              <div className="checkers__modal">
                ...
                <Modal title={showFirst ? "Уважаемый игрок, перед началом игры ознакомьтесь пожалуйста с правилами!" : "Правила игры"} onClose={() => setShow(false)} show={show}>
                  ...
                  <p>Выбор цвета игроками определяется жребием или по договоренности. Шашки расставляются на четырех, ближних к игроку, рядах на белых (светлых) клетках. Право первого хода обычно принадлежит игроку, который играет белым (светлым) цветом. Ходы осуществляются соперниками поочередно.</p>
                  <p>В начале игры все шашки соперников являются простыми. Простые шашки можно перемещать только вперед по диагоналям на соседнюю свободную клетку.</p>
                  <p>Если простая шашка дошла до последней горизонтали, она становится «дамкой» и обозначается переворачиванием. Дамка может перемещаться по диагоналям на любое количество свободных клеток.</p>
                  <p>Взятие шашки соперника производится, переносом через неё своей, в том случае, если она находится на соседней с простой шашкой диагональной клетке и за ней имеется свободное поле. Если после этого хода имеется продолжения для взятия, ход продолжается, при этом выбирается вариант по «правилу большинства», т.е. взятие наибольшего количества шашек соперника, в данном случае дамка не пользуется никакими преимуществами и не накладывает на игрока никаких дополнительных обязательств.</p>
                  <p>Взятие шашки соперника может производиться, как вперед, так и назад. Шашка (шашки) соперника снимается с доски.</p>
                  <p>Если простая шашка в процессе взятия шашек соперника достигает поля последней горизонтали и ей предоставляется возможность дальнейшего взятия по правилам боя дамкой, то она превращается в дамку, останавливаясь на поле последнего ряда. Право взятия по правилам дамки она приобретает лишь со следующего хода, если рядом не расположена шашка противника, в противном случае она имеет право забрать эту шашку и последующие шашки которые распологаются на одной клетке за ней, и если есть свободное пространство после взятой шашки.</p>
                </Modal>
              </div>
              <div className="board">
